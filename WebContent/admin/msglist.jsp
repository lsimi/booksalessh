...
<html lang="en">
  ...
  <body class="">
    ...
    <%	 CommonDAO dao = (CommonDAO)Info.getDao(request,"CommonDAO");
	 ...
	 String url = "/booksalessh/admin/msglist.jsp?1=1";
...
<div class="well">
   ...
   PageManager pageManager = PageManager.getPage("/booksalessh/admin/msglist.jsp?1=1",10, request);
   pageManager.doList(hql);
   PageManager bean= (PageManager)request.getAttribute("page");
   ArrayList<Chat> list=( ArrayList<Chat>)bean.getCollection(); 
  	for(Chat ct:list){
  		Member mbmap = (Member)dao.findById(ct.getMemberid(),"Member");
	    %>
        <tr>
          <td><%=ct.getMsg() %></td>
          <td><%=ct.getHfmsg() %></td>
          <td><%=mbmap.getTname() %></td>
          <td><%=ct.getSavetime() %></td>
          <td>
              <a href="admin/msgedit.jsp?id=<%=ct.getId() %>"><i class="icon-pencil"></i></a>
              <a href="admin/msglist.jsp?did=<%=ct.getId() %>" ><i class="icon-remove"></i></a>
          </td>
        </tr>
        <%} %>
      </tbody>
